...
export default function ModuleCard({ module, progress, onStart }) {
  ...
  const averageScenarioScore = scenarioScores.length > 0
    ? Math.round(scenarioScores.reduce((acc, s) => acc + s, 0) / scenarioScores.length)
    : 0;
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">{moduleTitle}</h3>
        ...
        <div className="mb-4">
          <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-2">{t('module_card.scenarios_label')}</h4>
          <div className="space-y-1">
            {moduleScenarios.slice(0, 2).map((scenario) => {
            ...
            {moduleScenarios.length > 2 && (
              <div className="text-xs text-gray-500 dark:text-gray-400 ml-4">
                {t('module_card.more_scenarios', { count: moduleScenarios.length - 2 })}
              </div>
            )}
          </div>
        </div>
        ...
        >
          {isCompleted ? t('module_card.completed') : t('module_card.start')}
        </button>

        {moduleScenarios.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
              <span>{t('module_card.progress')}</span>
              <span>{t('module_card.scenarios_count', { done: completedScenarios, total: moduleScenarios.length })}</span>
            </div>
            ...
            {completedScenarios > 0 && (
              <div className="mt-3 grid grid-cols-2 gap-4 text-xs">
                <div className="text-center">
                  ...
                  <div className="text-gray-500 dark:text-gray-400">{t('module_card.avg_score')}</div>
                </div>
                <div className="text-center">
                  <div className="font-semibold text-green-600 dark:text-green-400">
                    {Math.round(progress?.timeSpent / 1000 / 60) || 0}m
                  </div>
                  <div className="text-gray-500 dark:text-gray-400">{t('module_card.time_spent')}</div>
                </div>
              </div>
            )}
          </div>
